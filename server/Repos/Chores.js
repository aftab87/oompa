...
const createChoresPaths = (app) => {
  ...
  app.get("/Chores/:parent_uid", async (req, res) => {
    ...
    const populatedChores = await Promise.all(
      chores
        .map((chore) => chore.toObject())
        .map(async (chore) => {
  ...
  app.put("/chores/:id", async (req, res) => {
    ...
    if (parent_uid) chore.parent_uid = parent_uid;
    if (title) chore.title = title;
    if (points) chore.points = points;
    if (image) chore.image = image;
    if (kids) chore.kids = kids;
    if (start_date) chore.start_date = start_date;
    if (end_date) chore.end_date = end_date;
    if (repetition) chore.repetition = repetition;
