...
function RewardCard({ reward, date, time, refresh }) {
  console.log(reward);
        ...
        <div className={"card-body bg-white text-dark text-center d-flex flex-column gap-3" + (darkMode ? " bg-dark" : "")}>
          ...
          {user.type === "kid" && (
            ...
            <>
              ...
              {state === "available" && user.points>points &&  <div className="d-flex justify-content-center"> <Button onClick={claimReward}>Claim</Button></div>}
              {state === "claimed" && (
                <div>
          ...
          {user.type === "parent" && (
            ...
            <>
             ...
             {(claimed_kids.length < kids.length || kids.length===0) &&  <div className="d-flex justify-content-center gap-3">
                  <Button>Edit</Button>
              </div>}
              <div>{claimed_kids.length}/ {kids.length} kids claimed</div>
